...
contract Resource_management_contract {
    ...
    mapping(address => bool) public resourceServers; //增加resource server註冊resource set的權限
    mapping(bytes32 => ResourceSet) public resources;
    mapping(bytes32 => bool) public identifierCheck;
...
contract Authorization_contract {
    ...
    mapping(bytes32 => bool) public participantsOfIdentifier;
    mapping(address => bytes32) internal access_token;
    mapping(bytes32 => Policy_info) internal policies; //不同的identifier 應該要對應到不同的claim_info
    mapping(bytes32 => bytes32) public ticket_identifier; //ticket => identifier
    mapping(bytes32 => bytes32) public token_identifier;
    ...
    function requestAccessToken(bytes32 _ticket, string memory _claim) public {
        ...
        if(participantsOfIdentifier[checkParticipantHash] == true){
            ...
            token_vaildTime[access_token[msg.sender]] = now+ 20 seconds;
    ...
    function introspectAccessToken(
        ...
        //檢查token是否已經過期
        //require(now <= token_vaildTime[_token],"invalid_token");
